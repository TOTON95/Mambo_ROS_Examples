...
static const std::string OPENCV_ORIGINAL = "mambo camera window";		//Original image 
...
const double mambo_velocity = 0.07;						                //mambo general velocity
const double mambo_turn_velocity = 0.42;					            //mambo general turn velocity
...
cv::Point2f drone_center;							                    //Represents the center of the image, also the mambo's nose
...
geometry_msgs::Twist cmd_vel_mambo;						                //Variable that stores the linear velocity commands of mambo
...
void turn()
{
	cmd_vel_mambo.linear.x = 0;						                    //Turns the drone without translation
...
void hover()
{
	cmd_vel_mambo.linear.x = 0;						                    //Puts the drone in HOVER
	cmd_vel_mambo.linear.y = 0; 
	cmd_vel_mambo.linear.z = 0;
...
void move_drone(cv::Mat& image)
{
	...
	if (posX >= 0 && posY >= 0)
	{
		...
		{
			cmd_vel_mambo.angular.z = mambo_turn_velocity;						
		...
		{
			cmd_vel_mambo.linear.z = -mambo_velocity;
	...
	if(tracking_system_armed)
	{
		...
		{
			cmd_vel_mambo.linear.x = mambo_velocity;
			cmd_vel_pub_mambo.publish(cmd_vel_mambo);
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
	try		
	{
		...
		c_drone(final_img);						//Figures out mambo's center and draws a circle
		...
		{
			ROS_INFO("TAKE-OFF!!!");
			takeoff_mambo.publish(take_off);
		}			
		if(key == 'b')
		{
			ROS_INFO("LAND!!!");
...
{
	ros::init(argc,argv,"mambo_color_follower");				//Initialize the ROS node
	...
	//image_sub_= it_.subscribe("/usb_cam/image_raw",1,imageCallback);	//Subscribe to the mambo image topic
	image_sub_= it_.subscribe("/mambo_out",1,imageCallback);
	takeoff_mambo = nh_.advertise<std_msgs::Empty>("/mambo/take_off",1000);		//Publish data to the take-off topic
	land_mambo = nh_.advertise<std_msgs::Empty>("/mambo/land",1000);		//Publish data to the land topic
	cmd_vel_pub_mambo = nh_.advertise<geometry_msgs::Twist>("/mambo/cmd_vel",1000);	//Publish data to the movement topic
