...
/// Capture VICON Data
void getMamboPos(const geometry_msgs::TransformStamped::ConstPtr& pos)  
{
...
/// Get the signal to start the experiment
...
void getWaypoints(const Mambo_ROS_Examples::Waypoints::ConstPtr& msg)
{
    if(on_live)
    {
        ROS_WARN("\nAttention! Live mode deactivated!\n");
        on_live = false;
    }
...
int main(int argc, char** argv)
{
    ...
    sp_pos_X = n.advertise<std_msgs::Float64>("setpoint_pos_X",1);
    sp_pos_Y = n.advertise<std_msgs::Float64>("setpoint_pos_Y",1);
    ...
    while(n.ok())
    {
        ...
        mambo.rot_cmd_x = mambo.cmdX*cos(constrainAngle(hdg) * (PI/180)) + mambo.cmdY*sin(constrainAngle(hdg) * (PI/180));
        mambo.rot_cmd_y = mambo.cmdX*sin(constrainAngle(hdg) * (PI/180)) - mambo.cmdY*cos(constrainAngle(hdg) * (PI/180));
